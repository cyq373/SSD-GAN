...
def dense_norm(x, fmaps, **kwargs):
    if len(x.shape) > 2:
        x = tf.reshape(x, [-1, np.prod([d.value for d in x.shape[1:]])])
    w = get_weight([x.shape[1].value, fmaps], **kwargs)
    w = spectral_norm(tf.cast(w, x.dtype))
    return tf.matmul(x, w)
#----------------------------------------------------------------------------
# Convolutional layer.
...
def C_basic(
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    # i_gray = tf.image.rgb_to_grayscale(images_in)
    i_r, i_g, i_b = images_in[:,0,:,:], images_in[:,1,:,:], images_in[:,2,:,:]
    i_gray = 0.2989 * i_r + 0.5870 * i_g + 0.1140 * i_b
    i_gray_complex = tf.to_complex64(i_gray)
    i_fft = tf.fft2d(i_gray_complex) + 1e-10
    magnitude_spectrum = 20*tf.log(tf.abs(i_fft))
    ...
    radial_set = []
    for idx in range(4):
        tmp_spectrum = magnitude_spectrum[idx]
        tbin = tf.bincount(radius, tf.reshape(tmp_spectrum, [-1]))
        radial_prof = tbin / (nr + 1e-10)
        radial_prof = tf.reshape(radial_prof, [725])
        radial_set.append(radial_prof)
    radial_prof = tf.stack(radial_set, 0)
    rp_max = tf.reduce_max(radial_prof, 1, keepdims=True)
    rp_min = tf.reduce_min(radial_prof, 1, keepdims=True)
    radial_prof = (radial_prof - rp_min) / (rp_max - rp_min + 1e-10)

    with tf.variable_scope('fft'):
        scores_out = apply_bias(dense_norm(radial_prof, fmaps=max(label_size, 1), gain=1, use_wscale=use_wscale))

    assert scores_out.dtype == tf.as_dtype(dtype)
    scores_out = tf.identity(scores_out, name='fft_scores_out')
    return scores_out
