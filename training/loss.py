...
def GC_logistic_nonsaturating(G, D, C, opt, training_set, minibatch_size): # pylint: disable=unused-argument
    latents = tf.random_normal([minibatch_size] + G.input_shapes[0][1:])
    labels = training_set.get_random_labels_tf(minibatch_size)
    fake_images_out = G.get_output_for(latents, labels, is_training=True)
    fake_scores_out_main = fp32(D.get_output_for(fake_images_out, labels, is_training=True))
    fake_scores_out_fft = fp32(C.get_output_for(fake_images_out, labels, is_training=True)) * 0.4
    fake_scores_out_fix = (fake_scores_out_main + tf.stop_gradient(fake_scores_out_fft)) / 2
    loss = tf.nn.softplus(-fake_scores_out_fix)
    return loss
...
def C_hinge(G, C, opt, training_set, minibatch_size, reals, labels,): # pylint: disable=unused-argument
    latents = tf.random_normal([minibatch_size] + G.input_shapes[0][1:])
    fake_images_out = G.get_output_for(latents, labels, is_training=True)
    real_scores_out = fp32(C.get_output_for(reals, labels, is_training=True))
    fake_scores_out = fp32(C.get_output_for(fake_images_out, labels, is_training=True))
    
    real_scores_out_fft = autosummary('Loss/fftscores/realfft', real_scores_out)
    fake_scores_out_fft = autosummary('Loss/fftscores/fakefft', fake_scores_out)

    loss = tf.nn.relu(1.0 - real_scores_out_fft) + tf.nn.relu(1.0 + fake_scores_out_fft)
    return loss

def DC_logistic_simplegp(G, D, C, opt, training_set, minibatch_size, 
                         reals, labels, r1_gamma=10.0, r2_gamma=0.0): # pylint: disable=unused-argument
    latents = tf.random_normal([minibatch_size] + G.input_shapes[0][1:])
    fake_images_out = G.get_output_for(latents, labels, is_training=True)
    real_scores_out_main = fp32(D.get_output_for(reals, labels, is_training=True))
    fake_scores_out_main = fp32(D.get_output_for(fake_images_out, labels, is_training=True))
    real_scores_out_fft = fp32(C.get_output_for(reals, labels, is_training=True)) * 0.4
    fake_scores_out_fft = fp32(C.get_output_for(fake_images_out, labels, is_training=True)) * 0.4
    real_scores_out_main = autosummary('Loss/scores/real', real_scores_out_main)
    fake_scores_out_main = autosummary('Loss/scores/fake', fake_scores_out_main)
    real_scores_out_fft = autosummary('Loss/fftscores/realfft', real_scores_out_fft)
    fake_scores_out_fft = autosummary('Loss/fftscores/fakefft', fake_scores_out_fft)
    
    D_loss = tf.nn.softplus((fake_scores_out_main + tf.stop_gradient(fake_scores_out_fft)) / 2)  # -log(1 - logistic(fake_scores_out))
    D_loss += tf.nn.softplus(-(real_scores_out_main + tf.stop_gradient(real_scores_out_fft)) / 2)  # -log(logistic(real_scores_out)) # temporary pylint workaround # pylint: disable=invalid-unary-operand-type

    if r1_gamma != 0.0:
        with tf.name_scope('R1Penalty'):
            real_loss = opt.apply_loss_scaling(tf.reduce_sum(real_scores_out_main))
            real_grads = opt.undo_loss_scaling(fp32(tf.gradients(real_loss, [reals])[0]))
            r1_penalty = tf.reduce_sum(tf.square(real_grads), axis=[1,2,3])
            r1_penalty = autosummary('Loss/r1_penalty', r1_penalty)
        D_loss += r1_penalty * (r1_gamma * 0.5)

    if r2_gamma != 0.0:
        with tf.name_scope('R2Penalty'):
            fake_loss = opt.apply_loss_scaling(tf.reduce_sum(fake_scores_out_main))
            fake_grads = opt.undo_loss_scaling(fp32(tf.gradients(fake_loss, [fake_images_out])[0]))
            r2_penalty = tf.reduce_sum(tf.square(fake_grads), axis=[1,2,3])
            r2_penalty = autosummary('Loss/r2_penalty', r2_penalty)
        D_loss += r2_penalty * (r2_gamma * 0.5)
    
    C_loss = tf.nn.relu(1.0 - real_scores_out_fft) + tf.nn.relu(1.0 + fake_scores_out_fft)
