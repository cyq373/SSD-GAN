...
class MetricBase:
    ...
    def run(self, network_pkl, lod=None, run_dir=None, dataset_args=None, mirror_augment=None, num_gpus=1, tf_config=None, log_results=True):
        ...
        with tf.Graph().as_default(), tflib.create_session(tf_config).as_default(): # pylint: disable=not-context-manager
            ...
            if int(np.floor(lod)) == 0:
                _G, _D, _C, Gs = misc.load_pkl(self._network_pkl)
            else:
