...
if 1:
    ...
    C             = EasyDict(func_name='training.networks_stylegan.C_basic')               # Options for coordinator network.
    G_opt         = EasyDict(beta1=0.0, beta2=0.99, epsilon=1e-8)                          # Options for generator optimizer.
    D_opt         = EasyDict(beta1=0.0, beta2=0.99, epsilon=1e-8)                          # Options for discriminator optimizer.
    C_opt         = EasyDict(beta1=0.0, beta2=0.99, epsilon=1e-8)                          # Options for coordinator optimizer.
    G_loss        = EasyDict(func_name='training.loss.G_logistic_nonsaturating')           # Options for generator loss.
    GC_loss       = EasyDict(func_name='training.loss.GC_logistic_nonsaturating')          # Options for generator-coordinator loss.
    D_loss        = EasyDict(func_name='training.loss.D_logistic_simplegp', r1_gamma=10.0) # Options for discriminator loss.
    DC_loss       = EasyDict(func_name='training.loss.DC_logistic_simplegp', r1_gamma=10.0)# Options for discriminator-coordinator loss.
    C_loss        = EasyDict(func_name='training.loss.C_hinge')                            # Options for coordinator loss.
    ...
    sched.C_lrate_dict = EasyDict(sched.G_lrate_dict)
...
def main():
    ...
    kwargs.update(G_args=G, D_args=D, C_args=C, G_opt_args=G_opt, D_opt_args=D_opt, C_opt_args=C_opt)
    kwargs.update(G_loss_args=G_loss, GC_loss_args=GC_loss, D_loss_args=D_loss, DC_loss_args=DC_loss, C_loss_args=C_loss)
    kwargs.update(dataset_args=dataset, sched_args=sched, grid_args=grid, metric_arg_list=metrics, tf_config=tf_config)
    kwargs.update(resume_run_id=resume_run_dir, resume_kimg=resume_kimg)
